...
OBSTACLE_IMAGE = pygame.transform.scale(OBSTACLE_IMAGE, DEFAULT_IMAGE_SIZE)
class Background():
    def __init__(self):
        self.x = 0
        self.y = 0
    ...
    def draw(self):
        DISPLAYSURF.blit(self.image, (int(self.x), int(self.y)))
        DISPLAYSURF.blit(self.image, (int(self.x), int(self.y - self.height)))
    def update(self):
        self.y += self.speed
...
class Car():
    ...
    def draw(self):
        DISPLAYSURF.blit(CAR_IMAGE, (int(self.x), int(self.y)))
...
class Obstacles():
    ...
    def draw(self):
        for i in range(5):
            ...
            y = int(self.ls[i][1])
            DISPLAYSURF.blit(OBSTACLE_IMAGE, (x,y))
    def update(self):
        for i in range(5):
...
class Score():
    ...
    def update(self):
        self.score += 0.02
def gameStart(background):    #chuan bi mo game len
    background.__init__()
...
def isGameover(car, obstacles):
    ...
    for i in range(5):
        ...
        if rectCollision(carRect, obstaclesRect) == True:
            return True
    return False
def gamePLay(background, car, obstacles, score):     #phan choi chinh
    background.__init__()
    car.__init__()
    obstacles.__init__()
    moveLeft = False
    ...
    while True:
        ...
        pygame.display.update()
        fpsClock.tick(FPS)
def gameOver(background, car, obstacles, score):     #xuat hien khi thua mot man choi
    font = pygame.font.SysFont("consolas", 60)
    font = pygame.font.SysFont('consolas', 60)
...
def main():
    ...
    while True:
        gamePLay(background, car, obstacles, score)
        gameOver(background, car, obstacles, score)
